...
| `zylos add <org>/<repo>` works for any repo. A **bare name** (`zylos add foo`, `zylos search`) resolves through the registry: `zylos-ai/zylos-registry`'s `registry.json`, falling back to core's built-in list merged with the host's `~/zylos/.zylos/registry.json` (`{"components": {"<name>": {"repo": "<org>/<repo>", …}}}`). | `cli/lib/registry.js`, `cli/lib/components.js`, `cli/lib/config.js` `REGISTRY_FILE` |
